...
class Puzzle {
    public:
        ...
        void printPosValues(); // prints possible values 3D vector
    private:
        vector<vector <T> > grid; // from Sakai
        vector<vector <T> > startGrid; // copy of original grid
        vector<vector<vector <T> > > possibleVal; // possible values for each unknown location
};
...
Puzzle<T>::Puzzle(){ // default constructor
    ...
    // THE BIG CHANGES ARE BELOW!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


    // figure out possible values for each location
    rowNum=0;
    for (int m=0; m<9; m++) {
        for (int n=0; n<9; n++) {
            if (grid[m][n]!=0){ // if the spot on the original grid is already set
                cout << "grid[" << m << "][" << n << "] != 0 " << endl << endl;
                vector <T> oneDim; // one dimensional vector
                vector<vector <T> > row2; // similar to above row vector
                oneDim.push_back(grid[m][n]);
                row2.push_back(oneDim); // row2 is composed of 1D vectors
                possibleVal.push_back(row2); // the only possbile value is the 
                // value given by the original file
                //                row2.at(rowNum).push_back(grid[m][n]);
                //                possibleVal.at(rowNum).push_back(row2.at(rowNum)); 
            }
            else { // value on grid in this spot = 0, is up for grabs
                cout << "grid[" << m << "][" << n << "] == 0 " << endl;
// CHANGED FOLLOWING FOR LOOP FROM K=1 to 10 from K=0 to 9
                for (int k=1; k<10; k++) { // tests each possibility for the spot
                    if (rowValid(m,k)&&colValid(n,k)&&boxValid(m,n,k)) { // if this value is not in the row already
                        cout << "m=" << m << " n=" << n << " k=" << k << endl << endl;
                        vector <T> oneDim2; // one dimensional vector
                        vector<vector <T> > row3; // similar to row, row2

                        oneDim2.push_back(1); // pushes back current k value
                        row3.push_back(oneDim2);
                        possibleVal.push_back(row3);

                        /*                        row3.push_back(oneDim2); // row3 composed of 1D vectors
                                                  possibleVal.push_back(row3);
                                                  possibleVal.at(rowNum).push_back(row3.at(rowNum)); // pushes back this possbile value for this spot
                        //                    }
                        */
                }
                 else {
                        ...
                        oneDim2.push_back(0); // pushes back current k value
                        row3.push_back(oneDim2);
                        possibleVal.push_back(row3);
                 }
            }
